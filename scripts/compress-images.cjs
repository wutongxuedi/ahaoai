...
function getAllImages(dir) {
  ...
  for (const item of items) {
    ...
    } else if (/\.(jpg|jpeg|png)$/i.test(item.name) && !item.name.includes('.backup.')) {
      files.push(fullPath);
    }
  }
...
async function compressImage(filePath) {
  try {
    ...
    // 如果文件小于150KB，跳过压缩
    if (originalStats.size < 150 * 1024) {
      console.log(`⏭️  ${fileName}: ${originalSize}KB (已经足够小，跳过)`);
      return null;
    }
...
async function main() {
  ...
  if (results.length > 0) {
    ...
    console.log(`总共减少: ${(totalOriginal - totalCompressed).toFixed(2)}KB (${totalReduction}%)`);
  }
  
  console.log(`\n✨ 压缩完成！`);
  console.log(`💡 原始文件已备份为 .backup 扩展名`);
  console.log(`💡 如需恢复，可以删除压缩后的文件，将 .backup 文件改回原名`);
}
